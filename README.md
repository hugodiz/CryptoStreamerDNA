# CryptoStreamerDNA
Near-Time cryptocurrency data streaming functionality for Excel, powered by .NET > ExcelDNA
...
The refresh rate is dynamically adjusted so as to be as high as it reasonably can without risking exceeding the API policies on http request limits. Such limits are defined in terms of a maximum allowed 'request weight' per unit time (consecutively resetting after the unitary interval has passed). The RTD server (which powers the Excel RTD feeds) has a number of safety and adjustment measures to ensure the Binance API policies are never violated, regardless of the end-user's internet connection speed. These measures include:

- Automatic runtime reading and interpretation of API policy limits when the CryptoStreamer is first called upon during a new Excel session
- A negative feedback loop which imposes an artificial dynamic delay between requests, in order to steer the expected request weight usage to be around a safe percentage (currently set in the code as 75%) of the maximum allowed
- A built-in damping mechanism which waits progressively more time between calls if, despite the normal negative feedback loop from above, the request weight for this teim interval has exceeded the threshold considered safe (right now the code says this is 75%)
- A built-in 'last resort': an automatic routine of { cut-off -> cooling timer -> restart }, which kicks-in if (despite the measures above) the maximum allowed request weight has (just nearly) reached the limit. 

Note: the cooling routine **should not happen frequently**, beucase if it does happen recurrently, it means the code isn't doing a good enough job of keeping the average request weight / time interval below 75% -this would suggest the adjustment measures need improving. Right now, this would require a tweak to the source code, which although relatively simple, is not ideal - ideally, I will include a Ribbon input to specify a desired periodicity between requests (which translates to a certain average request weight / time interval, for any given ensemble of simultaneous Excel cell feeds). Although such a regulating input will never be allowed to cause the effective periodicity to be lower than what the RTD server deems sustainable, it will allow users to deliberately 'slow-down' the machine, which may be desirable in a variety of scenarios.
...
1. The CryptoStreamer keeps entering cooling down mode (which indicates I haven't been thorough enough in the code in order to avoid the streamer reaching that last resort)
